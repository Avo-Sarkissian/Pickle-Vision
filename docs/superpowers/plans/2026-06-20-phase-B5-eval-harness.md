...
## 10. Implementation tasks (expanded 2026-06-21)

Built now: the trajectory-fixture level is fully deterministic and device-free. The clip-fixture level (real footage + tap-test labels) is only scaffolded here (a Codable label sidecar); it is populated once Phase A produces real clips on-device. Each task is TDD in PickleVisionCore.

- B5.1 - `ReferenceBounce` ground-truth type `{ courtPoint: CGPoint (court feet), expectedVerdict: LineVerdict, time: TimeInterval }`. Make `LineVerdict` and `BallObservation` Codable (needed for JSON fixtures + clip sidecars). Tests: construction + Codable round-trip.
- B5.2 - `EvalRunner.evaluate(predicted: [JudgedBounce], reference: [ReferenceBounce]) -> EvalReport`. Greedy one-to-one match by time window (default 0.15 s) AND court proximity (default 3 ft); both are unverified knobs, flagged to tune on real clips. `EvalReport`: counts (reference/predicted/matched), bounce precision/recall (FP = unmatched predicted, FN = unmatched reference), clear-call bucket (correct / wrong-flip / declined), close-call bucket (correct-decline / confidently-decided), too-close rate, mean + max court error (feet) over matched pairs. Honesty rule (per section 6): a `tooCloseToCall` verdict on a near-line reference scores as correct, not a miss. Tests: perfect match, false positive, false negative, wrong flip, declined clear call, close honestly flagged, close decided, court-error distance, too-close rate.
- B5.3 - `TrajectoryFixture { name, imageCorners: [CGPoint], layout, observations: [BallObservation], reference: [ReferenceBounce] }` (Codable) + `courtModel()` (via CalibrationDraft) + end-to-end replay through `RefereeCore.evaluate` into `EvalRunner`. Tests: a hand-built two-bounce in/out fixture replays to a clean report; a hand-written JSON string decodes (proves low-friction labeling).
- B5.4 - `ClipLabel { clipID: UUID, reference: [ReferenceBounce] }` (Codable), aligned with the Phase-A SessionClip JSON-sidecar convention; round-trip test. Real clips populate this later.
- B5.5 - Fold in the B1 M6 coverage gaps as regression tests: Tracker empty/single-observation + gap-fill; BounceDetector multi-bounce + near-minProminence threshold; RefereeCore drops a non-finite (vanishing-line) bounce (the honest negative test the B1 review recommended).

Deferred (needs real clips / device): populating clip fixtures, capturing `ReferenceBounce` via the in-app tap-test, setting the works-on-court target rates (fixed against the first real clips, not invented up front), and any printed/markdown report polish.
